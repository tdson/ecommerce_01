class Admin::OrderProductsController < AdminController
  ...
  def update
    ...
    if @order_product.update_attributes(update_params) &&
      ...
      respond Settings.http_status[:success],
      ...
      respond Settings.http_status[:bad_request], [error_mgs, original_qty]
    end
  end
  ...
  def check_updateable
    ...
    unless @order.is_pending?
      flash.now[:warning] = t "orders.update.cannot_update"
      respond Settings.http_status[:bad_request],
        [t("orders.update.cannot_update"), @order_product.quantity]
    end
  end
end
